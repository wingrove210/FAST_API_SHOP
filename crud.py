...
async def main_number2(session: AsyncSession):
...
async def create_order(session: AsyncSession, promocode: str = None) -> Order:
    promocode = promocode or "default"  # Provide a default value if None
    order = Order(promocode=promocode, created_at=datetime.utcnow())
    session.add(order)
    await session.commit()
    return order
...
async def create_orders_and_products(session: AsyncSession):
    order_one = await create_order(session, promocode=None)
